dofile "Utility.lua"
--[[ 
********** MissileSidewinder by MJM ********** 
--]]

MissileSidewinder = class()
MissileSidewinder.maxParentCount = -1
MissileSidewinder.maxChildCount = -1
MissileSidewinder.connectionInput = sm.interactable.connectionType.logic + sm.interactable.connectionType.power
MissileSidewinder.connectionOutput = sm.interactable.connectionType.logic
MissileSidewinder.colorNormal = sm.color.new( 0xcb0a00ff )
MissileSidewinder.colorHighlight = sm.color.new( 0xee0a00ff )
MissileSidewinder.poseWeightCount = 1

MissileSidewinder.ammoEffectName = "MJM_Missile_Sidewinder"
MissileSidewinder.explosionEffectName = "MJM_ExplosionSmall"
MissileSidewinder.targetSpeed = 2.0 --2.0
MissileSidewinder.targetSpeedCam = 0.75 --0.75
MissileSidewinder.destructionLevel = 6
MissileSidewinder.destructionRadius = 2.0
MissileSidewinder.impulseRadius = 12.0
MissileSidewinder.impulseMagnitude = 40.0
MissileSidewinder.fuze = 1 -- delay before raycast starts looking for collision
MissileSidewinder.shotLifespan = 3000 -- ticks until destroys self
MissileSidewinder.explodesWhenExpires = true -- shot explodes if lifespan runs out?
MissileSidewinder.singleFire = true -- require pressing button for every shot?
MissileSidewinder.reloadTime = 120
MissileSidewinder.barrelOffset = 1.0 -- distance forward from center of gun to spawn projectile

-- ____________________________________ Server ____________________________________

function MissileSidewinder.server_onFixedUpdate( self, dt ) --- Server Fixed Update ------------
...
function MissileSidewinder.explode( self, loc )
...
function MissileSidewinder.server_updateClientTargetPos( self, targetPos )
...
function MissileSidewinder.client_onCreate( self ) --- Client setup ---
	print("* * * * * INIT MissileSidewinder * * * * *")
...
function MissileSidewinder.client_onRefresh( self )
	self:client_onCreate()
end

function MissileSidewinder.client_setTargetPos( self, targetPos )
	if not self.isCameraOwner then
		self.targetPos = targetPos
	end
end

function MissileSidewinder.client_onFixedUpdate( self, dt ) --- Client Fixed Update ------------
...
function MissileSidewinder.client_fire( self )
	...
	local detachVel = 0
	if self.shape.up.z < -0.5 then
		detachVel = 0.05
	elseif self.shape.up.z < 0.25 then
		detachVel = 0.02
	end
	local adjInitVel = toGlobal(self.shape, sm.vec3.new(barrelVel.x / 10, barrelVel.y, (barrelVel.z / 10) + detachVel))
...
function MissileSidewinder.client_onDestroy( self ) --- Cleanup ---
