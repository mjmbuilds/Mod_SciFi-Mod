...
function SelectFunction.open( _, self, callback, data )
	if self == nil or callback == nil or data == nil or data.partName == nil then return end
	...
	if not self.selectFunction_gui then
		local layoutName = data.partName:gsub("%s+", "") -- layout name should be recognizedFunctionsName without spaces
		self.selectFunction_gui = sm.gui.createGuiFromLayout(LAYOUTS_PATH..layoutName..'Inputs.layout')
	end
	self.selectFunction_gui:setOnCloseCallback("selectFunction_onGuiClose")
	for i = 1, #data.inputList do
		self.selectFunction_gui:setButtonCallback(data.inputList[i], "selectFunction_onButtonClick")
	end
